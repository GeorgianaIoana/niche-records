...
export function Footer() {
  return (
    <footer className="border-t-[0.5px] border-gold/20">
      <div className="max-w-7xl mx-auto px-6 lg:px-12 pt-20 pb-3 lg:pt-24 lg:pb-4">
        {/* Main footer content */}
        <div className="grid grid-cols-2 md:grid-cols-12 gap-12 lg:gap-8">
              ...
              {["All Products", "Vinyl", "CDs", "DVDs"].map((item) => (
                <li key={item}>
                  <Link
                    ...
                    className={`${questrial.className} text-[13px] text-white hover:text-gold transition-colors uppercase tracking-wider`}
                  >
                    {item}
                  </Link>
                </li>
          ...
          <div className="md:col-span-3 lg:col-span-2">
            ...
            <ul className={`${questrial.className} space-y-3 text-[13px] text-white uppercase tracking-wider`}>
              <li>
                <a
        ...
        {/* Bottom bar */}
        <div className="mt-10 pt-5 border-t border-white/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <p className={`${questrial.className} text-xs text-gray-600 tracking-wider`}>
            © {new Date().getFullYear()} {SITE_NAME}. All rights reserved.
          </p>
          <div className="flex gap-6">
            <Link
              href="#"
              className={`${questrial.className} text-xs text-gray-600 hover:text-gray-400 transition-colors tracking-wider`}
            >
              Privacy Policy
            </Link>
            <Link
